...
export default function PosPage({ user, onLogout }) {
  ...
  const builderTotalFillers = Object.values(builderState.fillers).reduce(
    (sum, q) => sum + q,
    0,
  );
  const builderBasePrice = builderState.bouquet
    ? Number(builderState.bouquet.price)
    : 0;

  const builderAdditionsPrice = Object.entries(builderState.additions).reduce(
    (sum, [id, qty]) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // This will now print the exact Laravel/MySQL error in your console!
      console.warn(
        "Backend error:",
      ...
      <div className="p-6">
        <header className="flex items-center gap-3 mb-6 max-w-[1600px] mx-auto">
          ...
          <h1 className="text-3xl font-bold text-gray-900">
            Pop-up Event Bouquet Builder
          </h1>

          <button
            onClick={() => navigate("/admin")} // Adjust to your actual admin route
            className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-5 py-2.5 rounded-xl font-bold shadow-sm hover:bg-gray-50 hover:text-blue-600 transition"
          >
            <LayoutDashboard className="w-5 h-5" /> View Admin Dashboard
          </button>
        </header>
          ...
          <div className="xl:col-span-8 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100 space-y-8">
            ...
            <section>
              <h2 className="text-xl font-bold text-gray-900 mb-5 flex items-center gap-3">
                <span className="bg-blue-600 text-white w-7 h-7 flex items-center justify-center rounded-full text-sm font-bold">
                  1
                </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {eventBases.map((b) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="font-bold text-lg text-gray-950 leading-tight">
                        {b.name}
                      </p>
                      <p className="text-sm text-gray-500 mt-0.5">
                        {b.description || "Event packaging size"}
                      </p>
            ...
            <section className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
              ...
              <p
                className={`text-sm mb-4 font-semibold px-3 py-1 rounded-full inline-block ${builderTotalFillers === 2 ? "bg-green-100 text-green-800" : "bg-blue-100 text-blue-700"}`}
              >
                Count: {builderTotalFillers} / 2{" "}
                {builderTotalFillers === 2 ? "(Max Reached)" : "(Required)"}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-2 gap-3">
                {fillers.map((f) => {
                  ...
                  return (
                    <div
                      key={f.id}
                      className="p-4 rounded-lg border border-gray-200 flex justify-between items-center bg-white shadow-sm"
                    >
                      <span className="text-sm font-bold text-gray-800 truncate pr-3">
                        {f.name}
                      </span>
                      <div className="flex items-center gap-2.5 flex-shrink-0">
                        <button
                          onClick={() => updateBuilderFiller(f.id, -1)}
                          disabled={qty === 0}
                          className="w-7 h-7 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-200 disabled:opacity-50"
                        >
                        ...
                        <button
                          onClick={() => updateBuilderFiller(f.id, 1)}
                          disabled={atMax}
                          className="w-7 h-7 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-600 hover:bg-blue-200 disabled:opacity-50"
                        >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-2 gap-3">
                {[...mainFlowers, ...fillers].map((a) => {
                  ...
                  return (
                    <div
                      key={a.id}
                      className="p-4 rounded-lg border border-gray-200 flex justify-between items-center bg-white shadow-sm hover:border-blue-200 transition"
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <span className="text-xs text-blue-600 font-bold">
                          +₱{Number(a.price).toLocaleString()} / stem
                        </span>
                      </div>
                      <div className="flex items-center gap-2.5 flex-shrink-0 ml-3">
                        <button
                          onClick={() => updateBuilderAddition(a.id, -1)}
                          disabled={qty === 0}
                          className="w-7 h-7 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-200 disabled:opacity-50"
                        >
                          <Minus className="w-4 h-4" />
                        ...
                        <button
                          onClick={() => updateBuilderAddition(a.id, 1)}
                          className="w-7 h-7 rounded-full bg-blue-100 border border-blue-200 flex items-center justify-center text-blue-600 hover:bg-blue-200"
                        >
                          <Plus className="w-4 h-4" />
                        </button>
                      </div>
              ...
              <div className="flex gap-4 items-center">
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-900">
                    ₱{builderBasePrice.toLocaleString()}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-900">
                    ₱{builderAdditionsPrice.toLocaleString()}
                  </p>
                </div>
          ...
          <div className="xl:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit flex flex-col max-h-[85vh] sticky top-6 z-10">
            <div className="flex justify-between items-center mb-6 border-b pb-4">
              ...
              {cart.length > 0 && (
                <button
                  onClick={clearCart}
                  className="text-red-500 flex items-center gap-1.5 text-sm font-semibold hover:text-red-700 transition"
                >
              ...
              {cart.length === 0 ? (
                <div className="text-center py-16 px-4 bg-blue-50/50 rounded-xl border border-blue-100">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Configure a bouquet to get started.
                  </p>
                </div>
              ) : (
                cart.map((item) => (
                  <div
                    key={item.id}
                    className="flex justify-between items-start gap-3 bg-gray-50 p-4 rounded-lg border"
                  >
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-gray-900 leading-tight">
                        Custom Event Bouquet
                      </h4>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center animate-in fade-in zoom-in duration-200">
            ...
            <div className="bg-gray-50 rounded-xl p-5 mb-8 border border-gray-100">
              ...
              <p className="text-4xl font-black text-green-600">
                ₱{successData.total.toLocaleString()}
              </p>
            </div>

            <button
              onClick={() =>
                setSuccessData({ show: false, total: 0, isDemo: false })
              }
