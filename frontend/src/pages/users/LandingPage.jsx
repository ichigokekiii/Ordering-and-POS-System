  ...
  const heroDefaults = [
    {
      ...
      title: "Carnations",
      subtitle:
        "Handcrafted and meticulously curated to convey your deepest sentiments.",
    },
    {
      image:
      ...
      title: "Sunflowers",
      subtitle:
        "Bright, vibrant, and bursting with joy. Perfect for lighting up any room.",
    },
    {
      image:
      ...
      title: "Roses",
      subtitle:
        "Timeless elegance and classic beauty for every romantic occasion.",
    },
  ];

  const heroData = heroDefaults.map((fallbackItem, index) => ({
    id: index,
    image: getCmsAssetUrl(
      getContentValue(`hero_image_${index + 1}`, fallbackItem.image),
    ),
    title: getContentValue(`hero_title_${index + 1}`, fallbackItem.title),
    subtitle: getContentValue(
      `hero_subtitle_${index + 1}`,
  ...
  const categoryCards = [
    {
      ...
      layoutClass:
        "group relative col-span-1 md:col-span-2 md:row-span-2 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer",
      imageClass:
        "w-full h-[350px] md:h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-105",
      titleClass:
        "text-white text-4xl md:text-5xl font-playfair font-bold pr-8",
    ...
    },
    {
      ...
      layoutClass:
        "group relative col-span-1 md:col-span-2 md:row-span-1 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer",
      imageClass:
        "w-full h-[250px] md:h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-105",
    ...
    },
    {
      ...
      layoutClass:
        "group relative col-span-1 md:col-span-1 md:row-span-1 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer",
      imageClass:
        "w-full h-[250px] md:h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-105",
      titleClass:
        "text-white text-xl md:text-2xl font-playfair font-bold leading-tight whitespace-pre-line",
      contentClass: "absolute bottom-6 left-6 md:bottom-8 md:left-8",
    },
    {
      ...
      layoutClass:
        "group relative col-span-1 md:col-span-1 md:row-span-1 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer bg-[#eaf2ff]",
      imageClass:
        "w-full h-[250px] md:h-full object-cover transition-transform duration-1000 ease-out group-hover:scale-105 mix-blend-multiply opacity-90",
      titleClass:
        "text-white text-xl md:text-2xl font-playfair font-bold leading-tight whitespace-pre-line",
      contentClass: "absolute bottom-6 left-6 md:bottom-8 md:left-8",
    },
    ...
    <div className="bg-[#fcfaf9] text-gray-900 min-h-screen pt-20 pb-12 overflow-x-hidden font-sans">
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_360px] gap-8 lg:gap-10 items-start mb-12">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-playfair font-bold leading-[1.15] text-gray-900 tracking-tight">
              <CmsEditableRegion
                ...
                field={getCmsField("home", "home_hero_title_intro")}
              ...
              <CmsEditableRegion
                cmsPreview={cmsPreview}
                field={getCmsField("home", "home_hero_title_accent")}
                className="inline-block"
              >
              ...
              <CmsEditableRegion
                cmsPreview={cmsPreview}
                field={getCmsField("home", "home_hero_title_outro")}
                className="inline-block"
              >
          ...
          >
            <div className="max-w-[320px] pr-2">
              <CmsEditableRegion
                cmsPreview={cmsPreview}
                field={getCmsField("home", "home_hero_cta_text")}
              >
              ...
              <div className="mt-8">
                ...
                >
                  ...
                  {/* Positioned exactly matching the expanding inner layer */}
                    ...
                    <CmsEditableRegion
                      cmsPreview={cmsPreview}
                      field={getCmsField("home", "home_hero_button_text")}
                      className="inline-block"
                    >
          ...
          {heroData.map((item, index) => {
            ...
            return (
              ...
              >
                <CmsEditableRegion
                  cmsPreview={cmsPreview}
                  field={getCmsField("home", `hero_image_${index + 1}`)}
                ...
                >
                  <img
                    src={item.image}
                    alt={item.title}
                    className={`h-full w-full object-cover transition-transform duration-[10s] ease-linear ${isActive ? "scale-110" : "scale-100"}`}
                  />
                </CmsEditableRegion>

                {/* Dark gradient overlay */}
                <div
                  className={`pointer-events-none absolute inset-0 z-20 bg-gradient-to-t from-black/90 via-black/30 to-transparent transition-opacity duration-700 ease-out ${isActive ? "opacity-100" : "opacity-60"}`}
                ></div>

                {/* Text Content */}
                <div
                  className={`relative z-30 w-full h-full p-6 md:p-10 text-white flex flex-col justify-end`}
                >
                  {/* Stable container width prevents text reflow during flex-grow */}
                  <div
                    className={`w-[80vw] md:w-[40vw] transition-opacity duration-700 ease-out ${isActive ? "opacity-100 delay-100" : "opacity-0 md:opacity-100"} ${!isActive && "md:hidden"}`}
                  >
                    <AnimatePresence mode="popLayout">
                      {isActive && (
                        ...
                        >
                          ...
                          >
                            <h2
                              className="text-4xl md:text-5xl lg:text-6xl font-playfair font-bold mb-4 drop-shadow-lg leading-tight"
                              style={{
                                color: getContentValue(
                                  "home_hero_card_title_color",
                                  "#ffffff",
                                ),
                              }}
                            >
                          ...
                          >
                            <p
                              className="text-sm md:text-lg lg:text-xl md:w-3/4 leading-relaxed font-light drop-shadow mb-8"
                              style={{
                                color: getContentValue(
                                  "home_hero_card_subtitle_color",
                                  "#f3f4f6",
                                ),
                              }}
                            >
                          ...
                          <Link
                            to="/products"
                            onClick={preventPreviewNavigation}
                            className="inline-block rounded-full bg-white text-gray-900 px-8 py-3.5 text-sm font-bold uppercase tracking-wider hover:bg-gray-900 hover:text-white transition-colors duration-300 shadow-xl"
                          >
                            {getContentValue(
                  ...
                  {!isActive && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                      ...
                      <h2
                        className="hidden md:block text-2xl lg:text-3xl font-playfair font-bold tracking-widest whitespace-nowrap -rotate-90 origin-center mix-blend-overlay shadow-sm"
                        style={{
                          color: getContentValue(
                            "home_hero_card_title_color",
                      ...
                      <h2
                        className="md:hidden absolute bottom-6 left-6 text-xl font-playfair font-bold tracking-wide drop-shadow-md"
                        style={{
                          color: getContentValue(
                            "home_hero_card_title_color",
                            "#ffffff",
      ...
      <section className="mx-auto max-w-[1400px] px-4 sm:px-8 mb-32">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          <CmsEditableRegion
            cmsPreview={cmsPreview}
            field={getCmsField("home", "home_featured_products_title")}
            className="inline-block"
          >
        ...
        <div className="flex flex-wrap justify-center gap-8 py-12">
          {[...products, ...premades].slice(0, 8).map((item) => (
            <div
              key={`${item.id}-${item.name || item.product_name}`}
              className="group cursor-pointer w-full sm:w-[48%] md:w-[30%] lg:w-[22%] max-w-[320px]"
            >
      ...
      <section className="bg-white py-24 border-y border-gray-100">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-8">
          <CmsEditableRegion
            cmsPreview={cmsPreview}
            field={getCmsField("home", "home_schedule_title")}
                  ...
                  <div className="absolute bottom-6 left-6 right-6 text-white">
                    ...
                    <p className="text-sm md:text-base text-white font-medium mb-4 leading-relaxed drop-shadow">
                      {schedule.description ||
                        `Join us for an exclusive floral showcase and arrangement session highlighting the ${schedule.schedule_name} collection.`}
                    </p>
      ...
      <section className="mx-auto max-w-[1400px] px-4 sm:px-8 mt-32 mb-20">
        <CmsEditableRegion
          cmsPreview={cmsPreview}
          field={getCmsField("home", "home_categories_title")}
          className="inline-block"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 md:gap-6 h-auto md:h-[700px]">
          {categoryCards.map((card) => (
            <div key={card.imageField} className={card.layoutClass}>
              <CmsEditableRegion
                cmsPreview={cmsPreview}
                field={getCmsField("home", card.imageField)}
                className="absolute inset-0 z-10 rounded-[inherit]"
                overlayClassName="rounded-[inherit]"
                persistentHint
              >
                <img
                  src={getCmsAssetUrl(
                    getContentValue(card.imageField, card.imageFallback),
                  )}
              ...
              <div className={`absolute z-30 ${card.contentClass}`}>
                {card.badge && (
                  <span className={`${card.badge === "Trending" ? "bg-[#4f6fa5]/90 text-white px-3" : "bg-white/90 text-gray-900 px-4"} text-xs font-bold py-1.5 rounded-full uppercase tracking-wider mb-3 inline-block backdrop-blur-sm shadow-sm`}>
                    {card.badge}
                  </span>
                )}
                <CmsEditableRegion
                  cmsPreview={cmsPreview}
                  field={getCmsField("home", card.titleField)}
                  className="relative z-40 inline-block"
                >
                ...
                {card.showHoverText && (
                  ...
                  >
                    <span className="text-white border-b border-white pb-1 font-semibold">
                      Shop to discover
                    </span>
                  </motion.div>
                )}
