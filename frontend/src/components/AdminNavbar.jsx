...
function AdminNavbar({ user, onLogout, onMenuToggle }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-[80] flex h-16 min-w-0 items-center justify-between gap-3 border-b border-gray-100 bg-white px-4 isolate md:gap-4 md:px-6 lg:px-8">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          <button
            type="button"
            onClick={onMenuToggle}
            className="flex min-h-11 min-w-11 shrink-0 items-center justify-center rounded-lg text-gray-600 transition-colors hover:bg-gray-100 hover:text-[#4f6fa5] lg:hidden"
          ...
          <div className="hidden min-w-0 flex-1 sm:block sm:max-w-md">
            <form onSubmit={handleSearchSubmit} className="relative">
              <input
                ...
                value={searchTerm}
                onChange={(event) =>
                  setSearchTerm(sanitizeSearchTerm(event.target.value))
                }
                maxLength={100}
                placeholder="What do you want to find?"
                className="w-full rounded-full border border-gray-100 bg-gray-100 py-2.5 pl-11 pr-4 text-sm text-gray-700 outline-none transition-all focus:border-[#4f6fa5] focus:bg-white focus:ring-2 focus:ring-[#eaf2ff]"
              />
        ...
        <div className="flex shrink-0 items-center gap-3 md:gap-6">
          ...
          <div className="border-l border-gray-100 pl-3 md:pl-6">
            <button
      ...
      {typeof document !== "undefined" && showNotifications
        ? createPortal(
            <div
              ...
              className="fixed max-w-[calc(100vw-2rem)] rounded-[1.5rem] border border-gray-100 bg-white p-4 shadow-xl"
      ...
      {typeof document !== "undefined" && showProfileMenu
        ? createPortal(
            <div
              ref={profilePanelRef}
              className="fixed max-w-[calc(100vw-2rem)] overflow-hidden rounded-[1.5rem] border border-gray-100 bg-white shadow-xl"
