        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] backdrop-blur-sm">
          <div className={`p-8 rounded-2xl w-[90%] max-w-[480px] shadow-2xl relative ${dm ? "bg-gray-800" : "bg-white"}`}>
            <button
              ...
              className={`absolute top-4 right-4 rounded-full p-2 transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "text-gray-400 hover:text-gray-200 bg-gray-700 hover:bg-gray-600" : "text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100"}`}
            >
            ...
            <div className="grid grid-cols-2 gap-6">
              <button
                ...
                disabled={isSubmittingTransaction}
                className={`aspect-square border-2 rounded-2xl flex flex-col items-center justify-center gap-4 transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "bg-blue-900/30 hover:bg-blue-900/50 border-blue-700 text-blue-300" : "bg-blue-50 hover:bg-blue-100 border-blue-200 text-blue-700"}`}
              >
              ...
              <button
                ...
                disabled={isSubmittingTransaction}
                className={`aspect-square border-2 rounded-2xl flex flex-col items-center justify-center gap-4 transition-all active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "bg-purple-900/30 hover:bg-purple-900/50 border-purple-700 text-purple-300" : "bg-purple-50 hover:bg-purple-100 border-purple-200 text-purple-700"}`}
              >
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] backdrop-blur-sm">
          <div className={`rounded-2xl w-[90%] max-w-[600px] shadow-2xl flex flex-col overflow-hidden ${dm ? "bg-gray-800" : "bg-white"}`}>
            <div className={`border-b p-4 flex items-center gap-4 ${dm ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
              <button disabled={isSubmittingTransaction} onClick={() => { setCashModal(false); setMethodModal(true); }} className={`p-2 rounded-full transition disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "text-gray-400 hover:text-gray-200 hover:bg-gray-700" : "text-gray-500 hover:text-gray-800 hover:bg-gray-100"}`}>
              ...
              <div className="grid grid-cols-3 gap-3">
                {quickAmounts.map(amt => (
                  <button
                    ...
                    disabled={isSubmittingTransaction}
                    className={`border py-3 rounded-xl font-bold text-lg shadow-sm transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "bg-gray-700 border-gray-600 text-gray-200 hover:text-blue-300 hover:border-blue-600" : "bg-white border-gray-200 text-gray-700 hover:text-blue-700 hover:border-blue-300"}`}
                  >
                    +₱{amt}
                  </button>
                ))}
                <button
                  ...
                  disabled={isSubmittingTransaction}
                  className={`border py-3 rounded-xl font-bold text-lg shadow-sm transition active:scale-95 col-span-1 text-red-500 disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "bg-gray-700 border-gray-600 hover:bg-red-900/30" : "bg-white border-gray-200 hover:bg-red-50"}`}
                >
                  Clear
                </button>
                <button
                  ...
                  disabled={isSubmittingTransaction}
                  className="bg-blue-600 border border-blue-600 hover:bg-blue-700 py-3 rounded-xl font-bold text-lg text-white shadow-sm transition active:scale-95 col-span-2 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Exact Amount (₱{total})
                </button>
              </div>
            ...
            <div className={`p-4 border-t ${dm ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
              <button
                ...
                disabled={cashReceived < total || isSubmittingTransaction}
                className={`w-full py-4 rounded-xl font-bold text-xl transition-all shadow-sm ${
                  cashReceived >= total && !isSubmittingTransaction
              ...
              >
                {isSubmittingTransaction ? "Processing..." : "Finish Transaction"}
              </button>
            </div>
          </div>
        </div>
      ...
      {qrModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] backdrop-blur-sm">
          <div className={`rounded-2xl w-[90%] max-w-[400px] shadow-2xl flex flex-col overflow-hidden ${dm ? "bg-gray-800" : "bg-white"}`}>
            <div className={`border-b p-4 flex items-center gap-4 ${dm ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"}`}>
              <button disabled={isSubmittingTransaction} onClick={() => { setQrModal(false); setMethodModal(true); }} className={`p-2 rounded-full transition disabled:cursor-not-allowed disabled:opacity-50 ${dm ? "text-gray-400 hover:text-gray-200 hover:bg-gray-700" : "text-gray-500 hover:bg-gray-100 hover:text-gray-800"}`}>
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
              </button>
